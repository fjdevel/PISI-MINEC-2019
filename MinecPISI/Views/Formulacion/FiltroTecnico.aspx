...
<asp:Content ID="Content3" ContentPlaceHolderID="bodyContentBlank" runat="server">
    ...
    <div class="text-white p-2 text-center fixed-top mt-5 row">
        <div class="col-md-10"></div>
        <div data-toggle="tooltip" data-placement="top" title="Detalle de la iniciativa" ><i class="btn btn-info far fa-3x fw fa-file-alt shadow" data-toggle="modal" data-target="#DetalleModal"></i></div>
    </div>
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                    <asp:GridView
                        ID="GVCriteriosPertinencia"
                        runat="server"
                        AutoGenerateColumns="False"
                        DataKeyNames="IdCampo"
                        CssClass="table table-bordered"
                        OnRowDataBound="GVCriteriosPertinencia_RowDataBound"
                        ShowFooter="true">
                        <Columns>
                            ...
                            <asp:TemplateField HeaderText="Puntaje">
                                <ItemTemplate>
                                    <asp:TextBox ID="TxtPuntaje" data-campo='<%#Eval("IdCampo") %>' required="true" CssClass="textboxper" runat="server"  max='<%# Eval("PuntajeMaximo") %>' min="0" Text='<%# Bind("Puntaje") %>' old='<%# Eval("Puntaje") %>'></asp:TextBox>
                                </ItemTemplate>
                            </asp:TemplateField>
                        </Columns>
                    </asp:GridView>
            ...
            <div class="table-responsive">
                    <asp:GridView
                        ...
                        DataKeyNames="IdCampo"
                        CssClass="table table-bordered"
                        OnRowDataBound="GVInnovacion_RowDataBound"
                        ShowFooter="true"
                        >
                        <Columns>
                            <asp:BoundField HeaderText="Criterio" DataField="Criterio" />
                            ...
                            <asp:TemplateField HeaderText="Puntaje">
                                <ItemTemplate>
                                    <asp:TextBox ID="TxtPuntaje" data-campo='<%#Eval("IdCampo") %>' required="true" CssClass="textboxinn" runat="server" max='<%# Eval("PuntajeMaximo") %>' min="0" Text='<%# Bind("Puntaje") %>' old='<%# Eval("Puntaje") %>'></asp:TextBox>
                                </ItemTemplate>
                            </asp:TemplateField>
                        </Columns>
                    </asp:GridView>
        ...
        <div class="text-center mt-4 mb-4">
            <asp:Button id="BtnGuardar" runat="server" class="btn btn-success fas fa-check" type="submit" OnClick="GuardarFiltroClick" Text="Guardar"/>
            <asp:LinkButton id="BtnAprobar" runat="server" cssClass="btn btn-success btn-icon-split mb-1 aprobar" type="submit" UseSubmitBehavior="True" OnClick="AprobarClick">
                ...
                <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label>Observaciones</label>
                             <asp:TextBox ID="txt_observa" TextMode="MultiLine" CssClass="form-control" placeholder="Ingresa las Observaciones que tienes en el registro del proyecto" runat="server" MaxLength="200" />
                        </div>
...
<asp:Content ID="Content4" ContentPlaceHolderID="jsextra" runat="server">
    ...
    <script>
        $(() => {
            ...
            for (let i = 0; i < $(".textboxinn").length; i++) {
                if ($(".textboxinn")[i].value === "")
                    $(".textboxinn")[i].value = 0;
            }
            sumar();
        ...
        function sumar() {
            let suma = 0;
            for (let i = 0; i < $(".textboxper").length; i++) {
                suma += parseInt($(".textboxper")[i].value);
            }
            $(".lbltotalper")[0].value = suma;
            suma = 0;
            for (let i = 0; i < $(".textboxinn").length; i++) {
                suma += parseInt($(".textboxinn")[i].value);
            }
            $(".lbltotalinn")[0].value = suma;
            ComprobarTotales();
        }
        function ComprobarTotales() {
            if ($(".lbltotalper")[0].value >= 7 && $(".lbltotalinn")[0].value >= 7)
                $('#bodyContentBlank_BtnAprobar').removeClass('disabled');
            else
                $('#bodyContentBlank_BtnAprobar').addClass('disabled');
        }
        $(".textboxper").change(function () {
            ComprobarLimites($(this)[0]);
            sumar();
        });
         $(".textboxinn").change(function () {
            ComprobarLimites($(this)[0]);
            sumar();
        });
    ...
    <script>
        ...
        $(() => {
         ...
         var cronograma = new $.Cronograma($('#table-cron2'), {
            ...
            var mymap = L.map('map2').setView([$('#bodyContentBlank_location2').val().split(",")[0],$('#bodyContentBlank_location2').val().split(",")[1]], 15);
